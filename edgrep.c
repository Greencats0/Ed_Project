...
unsigned int* address(void) {  int sign,opcnt, nextopand, c;
  unsigned int *a, *b;
...
void compile(int eof) {  int c, cclcnt;
  ...
  lastep = 0;
  for (;;) {
    if (ep >= &expbuf[ESIZE]) { /*goto cerror;*/break; }
    c = getchr();
    if (c == '\n') { peekc = c;  c = eof; }
    if (c==eof) { if (bracketp != bracket) { /*goto cerror;*/break; }
    *ep++ = CEOF;
    return;  }
    if (c!='*') { lastep = ep; }
    switch (c) {
      case '\\':
        if ((c = getchr())=='(') {
          if (nbra >= NBRA) { /*goto cerror;*/break; }
          *bracketp++ = nbra;
          *ep++ = CBRA;
          *ep++ = nbra++;
          continue;
        }
        if (c == ')') {  if (bracketp <= bracket) { /*goto cerror;*/break; }
        *ep++ = CKET;
      ...
      case '\n':  /*goto cerror;*/break;
      case '*':  if (lastep==0 || *lastep==CBRA || *lastep==CKET) { /*goto defchar;*/break; }
      *lastep |= STAR;
      continue;
