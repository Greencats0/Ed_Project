...
void compile(int eof) {  int c, cclcnt;  char *ep = expbuf, *lastep, bracket[NBRA], *bracketp = bracket;
  ...
  for (;;) {
    if (ep >= &expbuf[ESIZE]) { /*goto cerror;*/break; }  c = getchr();  if (c == '\n') { peekc = c;  c = eof; }
    if (c==eof) { if (bracketp != bracket) { /*goto cerror;*/break; }  *ep++ = CEOF;  return;  }
    if (c!='*') { lastep = ep; }
    switch (c) {
      case '\\':
        if ((c = getchr())=='(') {
          if (nbra >= NBRA) { /*goto cerror;*/break; }  *bracketp++ = nbra;  *ep++ = CBRA;  *ep++ = nbra++;  continue;
        }
        if (c == ')') {  if (bracketp <= bracket) { /*goto cerror;*/break; }  *ep++ = CKET;  *ep++ = *--bracketp;  continue; }
        if (c>='1' && c<'1'+NBRA) { *ep++ = CBACK;  *ep++ = c-'1';  continue; }
        *ep++ = CCHR;  if (c=='\n') { /*goto cerror;*/ break;}  *ep++ = c;  continue;
      case '.': *ep++ = CDOT;  continue;
      case '\n':  /*goto cerror;*/break;
      case '*':  if (lastep==0 || *lastep==CBRA || *lastep==CKET) { /*goto defchar;*/break; }  *lastep |= STAR; continue;
      case '$':  if ((peekc=getchr()) != eof && peekc!='\n') { /*goto defchar;*/break; }  *ep++ = CDOL;  continue;
      case '[':  *ep++ = CCL;  *ep++ = 0;  cclcnt = 1;  if ((c=getchr()) == '^') {  c = getchr();  ep[-2] = NCCL; }
        do {
          if (c=='\n') { /*goto cerror;*/ break;}  if (c=='-' && ep[-1]!=0) {
            if ((c=getchr())==']') { *ep++ = '-';  cclcnt++;  break; }
            while (ep[-1] < c) {  *ep = ep[-1] + 1;  ep++;  cclcnt++;  if (ep >= &expbuf[ESIZE]) { /*goto cerror;*/ break;} }
          }
          *ep++ = c;  cclcnt++;  if (ep >= &expbuf[ESIZE]) { /*goto cerror;*/ break;}
        } while ((c = getchr()) != ']');
